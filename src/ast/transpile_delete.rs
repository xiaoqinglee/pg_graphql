//! AST-based transpilation for DeleteBuilder
//!
//! This module implements the ToAst trait for DeleteBuilder, converting it
//! to a type-safe AST that can be rendered to SQL.

use super::{
    build_connection_subquery, build_filter_expr, build_relation_subquery_expr, coalesce,
    column_ref, count_star, empty_jsonb_array, func_call, func_call_schema, jsonb_agg,
    jsonb_build_object, string_literal, AstBuildContext, ToAst,
};
use crate::ast::{
    BinaryOperator, CaseExpr, ColumnRef, Cte, CteQuery, DeleteStmt, Expr, FromClause, Ident,
    Literal, ParamCollector, SelectColumn, SelectStmt, Stmt,
};
use crate::builder::{
    DeleteBuilder, DeleteSelection, FunctionBuilder, FunctionSelection, NodeBuilder, NodeSelection,
};
use crate::error::GraphQLResult;

/// The result of transpiling a DeleteBuilder to AST
pub struct DeleteAst {
    /// The complete SQL statement
    pub stmt: Stmt,
}

impl ToAst for DeleteBuilder {
    type Ast = DeleteAst;
    ...
    fn to_ast(&self, params: &mut ParamCollector) -> GraphQLResult<Self::Ast> {
        ...
        // Build the DELETE statement for the 'impacted' CTE
        ...
        let stmt =
            build_delete_with_at_most(delete_stmt, select_columns, &block_name, self.at_most);

        Ok(DeleteAst { stmt })
    }
}

/// Build the select columns for jsonb_build_object from DeleteSelection
fn build_select_columns(
    ...
    for selection in selections {
        match selection {
            DeleteSelection::AffectedCount { alias } => {
            ...
            DeleteSelection::Typename { alias, typename } => {
...
/// Build the full DELETE with at_most check using CTEs
fn build_delete_with_at_most(
    delete_stmt: DeleteStmt,
    select_columns: Vec<(Expr, Expr)>,
    block_name: &str,
    at_most: i64,
) -> Stmt {
    // CTE 1: impacted AS (DELETE ...)
    let impacted_cte = Cte {
        name: Ident::new("impacted"),
        columns: None,
        query: CteQuery::Delete(delete_stmt),
        materialized: None,
    };
        ...
        vec![(
            ...
            Expr::Cast {
                expr: Box::new(func_call_schema(
                    "graphql",
                    "exception",
                    vec![string_literal("delete impacts too many records")],
                )),
